...
const OrderPage = () => {
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchOrders(newPage, pageSize, sortColumn, sortDirection, {
  ...
  const handlePageSizeChange = (size: number) => {
    ...
    fetchOrders(1, size, sortColumn, sortDirection, {
      customerId: selectedCustomer?.customerId,
      status: statusFilter,
      startDate,
      endDate,
  ...
  useEffect(() => {
    const fetchCustomersAndOrders = async () => {
      try {
        ...
        // Fetch orders
        await fetchOrders(currentPage, pageSize, sortColumn, sortDirection, {
          customerId: selectedCustomer?.customerId,
          status: statusFilter,
          startDate,
          endDate,
  ...
  const handleStatusChange = async (orderId: string) => {
    ...
    try {
      ...
      fetchOrders(currentPage, pageSize, sortColumn, sortDirection, {
        customerId: selectedCustomer?.customerId,
        status: statusFilter,
        startDate,
        endDate,
  ...
  const handleSort = (column: string) => {
    ...
    fetchOrders(currentPage, pageSize, column, newSortDirection, {
      customerId: selectedCustomer?.customerId,
      status: statusFilter,
      startDate,
      endDate,
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">{m('loading')}</div>
    );
  }
              ...
              <tbody>
                {filteredOrders?.map((order) => (
                  <tr key={order.id} className="border-b hover:bg-gray-100">
                    ...
                    <td
                      className={` py-3 px-4 text-center ${order.status === 'Completed'
                          ? 'bg-green-300'
                    ...
                    <td className=" py-3 px-4 text-center">
                      <div className="flex justify-center space-x-2 items-center">
                        <select
                          ...
                          onChange={(e) =>
                            setEditableStatus({ ...editableStatus, [order.id]: e.target.value })
                          }
                        >
